...
export default function EnhancedHero({ onJoinClick, onWatchDrawClick }) {
  ...
  const cardRotateX = useSpring(useTransform(mouseY, [-300, 300], [10, -10]), {
    stiffness: 100,
    damping: 20,
  });
  const cardRotateY = useSpring(useTransform(mouseX, [-300, 300], [-10, 10]), {
    stiffness: 100,
    damping: 20,
  });
    ...
    const draw = () => {
      ...
      particles.forEach((p) => {
        ...
        if (p.type === "rupee") {
          ...
          for (let i = 0; i < 5; i++) {
            ctx.lineTo(
              Math.cos(((18 + i * 72) * Math.PI) / 180) * (p.size / 2),
              Math.sin(((18 + i * 72) * Math.PI) / 180) * (p.size / 2),
            );
            ctx.lineTo(
              Math.cos(((54 + i * 72) * Math.PI) / 180) * (p.size / 4),
              Math.sin(((54 + i * 72) * Math.PI) / 180) * (p.size / 4),
            );
          }
          ctx.closePath();
          ctx.fill();
          ...
          setRecentDraws((prev) => [
            {
              id: randomID,
              status: "Paid",
              pool: `#09${Math.floor(4 + Math.random() * 5)}`,
            },
            ...prev.slice(0, 2),
          ]);
        }
        ...
        {user && (
          ...
          >
            <p className="text-white text-sm font-bold text-center">
              Welcome back,{" "}
              <span className="text-[#f2c12e]">{displayName}</span>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-16 items-center relative z-10 w-full">
          ...
          >
            <motion.div
              ...
              className="inline-flex items-center gap-2 bg-blue-500/10 border border-blue-500/20 px-3.5 py-1.5 rounded-full w-fit mb-6 shadow-[0_0_15px_rgba(59,130,246,0.1)]"
            >
              ...
              <span className="text-xs font-semibold text-blue-400 uppercase tracking-widest">
                Stackr Savings Slots Now Active
              </span>
            </motion.div>
            ...
            <motion.h1
              variants={itemVariants}
              className="text-5xl md:text-8xl font-black tracking-tight leading-[1.05] text-white mb-6"
            >
              Save Small. <br />
              <span className="text-[#f2c12e] bg-gradient-to-r from-[#f2c12e] via-[#ffe494] to-[#fcd96d] bg-clip-text text-transparent drop-shadow-[0_2px_10px_rgba(242,193,46,0.15)]">
                Receive Big.
              </span>
            </motion.h1>
            ...
            <motion.h2
              variants={itemVariants}
              className="text-xl md:text-2xl font-bold text-slate-200 tracking-wide mb-4 flex items-center gap-2"
            >
            ...
            <motion.p
              variants={itemVariants}
              className="text-slate-400 text-base md:text-xl max-w-xl mb-10 leading-relaxed"
            >
              Pay just{" "}
              <span className="text-white font-semibold underline decoration-[#f2c12e] decoration-2">
                ₹2,000 monthly
              </span>
              . Step into highly transparent, provably-fair live cycles to take
              home a massive pool of{" "}
              <span className="text-[#f2c12e] font-extrabold">₹50,000</span>{" "}
              early.
            </motion.p>
            ...
            >
              <motion.button
                onClick={() =>
                  (window.location.href = "https://tally.so/r/dWQL6N")
                }
                whileHover={{
                  scale: 1.04,
              ...
              <motion.button
                onClick={() =>
                  window.open(
                    "https://youtube.com/live/oji3H3snGQc?feature=share",
                    "_blank",
                    "noopener,noreferrer",
                  )
                }
                ...
                whileTap={{ scale: 0.97 }}
                className="w-full sm:w-auto bg-transparent text-white font-bold px-7 py-4.5 rounded-xl border-2 border-slate-700 transition-all duration-200 flex items-center justify-center gap-3 group"
              >
                <svg
                  className="w-5 h-5 text-slate-400 group-hover:text-[#f2c12e] group-hover:scale-110 transition-transform fill-none stroke-current"
                  viewBox="0 0 24 24"
                >
            ...
            <motion.div
              variants={itemVariants}
              className="flex items-center gap-4 bg-[#06122c]/40 border border-slate-800/60 p-3.5 rounded-2xl max-w-sm backdrop-blur-xl"
            >
              <div className="flex -space-x-2.5 overflow-hidden">
                {[
                  ...
                  <img
                    key={i}
                    className="inline-block h-9 w-9 rounded-full ring-2 ring-[#010614] object-cover"
                    src={src}
                    alt="Pool Member"
                  />
                ))}
              ...
              <div className="flex flex-col">
                <p className="text-xs text-white font-bold">
                  Over 2.5 Lakhs Pooled since Today
                </p>
                <p className="text-[11px] text-slate-400 font-medium">
                  Joined by 100+ verified Indian savers
                </p>
              </div>
            </motion.div>
          </motion.div>
          ...
          <motion.div
            ...
            className="lg:col-span-5 hidden lg:flex justify-center relative perspective-[1000px]"
          >
            <div className="w-[400px] bg-gradient-to-b from-[#081533] to-[#030a1a] rounded-[40px] border-[3px] border-slate-800/80 p-6 shadow-[0_30px_70px_-10px_rgba(0,0,0,0.9)] relative overflow-hidden backdrop-blur-md group">
              ...
              <div className="flex justify-between items-center mb-6 border-b border-slate-800/60 pb-4">
                <div className="flex items-center gap-2">
                  <div className="bg-[#f2c12e] w-5 h-5 rounded-md text-[11px] font-black text-black flex items-center justify-center shadow-md">
                    S
                  </div>
              ...
              <div className="bg-[#040e24] border border-slate-800/90 rounded-2xl p-4 mb-4 relative">
                ...
                <div className="grid grid-cols-4 gap-2.5 text-center">
                  {[
                    ...
                    <div
                      key={i}
                      className="bg-[#081533] p-2.5 rounded-xl border border-slate-800/40 relative group/time"
                    >
              ...
              <div className="bg-gradient-to-r from-[#061535] to-[#040d21] border border-slate-800/90 rounded-2xl p-5 mb-4 relative overflow-hidden flex justify-between items-center shadow-inner">
                <div className="z-10">
                  <span className="text-xs text-slate-400 block font-bold tracking-wide mb-0.5">
                    Yield-Augmented Prize
                  </span>
                  <span className="text-4xl font-black text-[#f2c12e] tracking-tighter drop-shadow-md">
                    ₹50,000
                  </span>
                  <p className="text-[10px] text-slate-400 mt-2 font-medium">
                    Guaranteed payout to drawn member
                  </p>
                </div>
                ...
                <div className="w-16 h-16 bg-[#0a1b3f] rounded-2xl flex items-center justify-center border border-slate-700/50 shadow-md relative group-hover:rotate-6 transition-transform duration-300">
                  ...
                  <svg
                    className="w-9 h-9 text-[#f2c12e]"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M19 5h-2V3H7v2H5c-1.1 0-2 .9-2 2v3c0 2.42 1.72 4.44 4.00 4.90C7.79 14.12 9.69 15 12 15s4.21-.88 4.99-2.10C19.28 12.44 21 10.42 21 8V7c0-1.1-.9-2-2-2zM5 10V7h2v3H5zm14 0h-2V7h2v3zm-7 7c-1.66 0-3-1.34-3-3h6c0 1.66-1.34 3-3 3z" />
              ...
              <div className="bg-[#040e24]/90 rounded-2xl p-4.5 border border-slate-800/50 text-xs flex flex-col gap-3 backdrop-blur-md">
                <div className="flex justify-between items-center border-b border-slate-800/60 pb-2.5">
                  ...
                  <span className="text-emerald-400 font-extrabold bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-0.5 rounded-md text-[10px]">
                    Verified Active
                  </span>
                </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center relative z-10">
          ...
          <div className="lg:col-span-5 text-center lg:text-left">
            ...
            <p className="text-slate-400 text-sm md:text-base leading-relaxed max-w-md mx-auto lg:mx-0">
              No behind-the-door decisions. When the timer strikes zero, an
              encrypted decentralized poll selects the winning Unique ID live.
              Watch the ticket visualizer cycle through transparent allocations.
            </p>
            ...
            <div className="flex-1 bg-[#020a1f] border-2 border-slate-800 rounded-3xl p-6 flex flex-col justify-between shadow-[0_20px_40px_rgba(0,0,0,0.5)] relative overflow-hidden">
              ...
              <div className="flex justify-between items-center mb-6">
                <span className="text-[11px] font-mono text-slate-500 tracking-widest uppercase">
                  ID Poll Scraper V2.0
                </span>
                <span
                  className={`text-[10px] px-2 py-0.5 rounded font-bold transition-all ${isDrawing ? "bg-amber-500/20 text-amber-400 animate-pulse" : "bg-emerald-500/20 text-emerald-400"}`}
                >
              ...
              <div className="my-8 text-center bg-[#051438] border border-slate-800/80 rounded-2xl py-8 px-4 shadow-inner relative">
                  ...
                  >
                    {drawnTicket.split("-").map((segment, i) => (
                      <span
                        key={i}
                        className={
                          i === 1 ? "text-[#f2c12e]" : "text-slate-300"
                        }
                      >
                        {segment}
                        {i < 2 ? "-" : ""}
                ...
                <p className="text-[10px] text-slate-500 font-medium mt-3 tracking-wide uppercase">
                  {isDrawing
                    ? "Hashing active network slots..."
                    : "Matching Verification Hash string"}
                </p>
              </div>
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-16">
            <div className="inline-flex items-center gap-2 bg-[#f2c12e]/10 border border-[#f2c12e]/20 px-3.5 py-1.5 rounded-full w-fit mb-4">
              <span className="text-xs font-bold text-[#f2c12e] uppercase tracking-widest">
                Real Payouts, Proven On-Chain
              </span>
            </div>
            ...
            <p className="text-slate-400 mt-4 max-w-xl mx-auto text-base md:text-lg">
              No guesswork. Regular transparent cycles distributing funds
              instantly into bank accounts.
            </p>
          </div>
              ...
              <div className="group relative bg-[#040e24] border border-slate-800/80 rounded-[32px] p-4 flex flex-col justify-between overflow-hidden shadow-2xl transition-all duration-300 hover:border-slate-700 hover:shadow-[0_20px_50px_rgba(0,0,0,0.6)]">
                ...
                <div className="px-2 pb-2 flex-grow flex flex-col justify-between">
                  <div className="relative">
                    <span className="absolute -top-3 -left-2 text-5xl font-serif font-black text-slate-800/40 select-none pointer-events-none">
                      “
                    </span>
                    <p className="text-xs md:text-sm text-slate-300 leading-relaxed font-medium relative z-10 pl-4 italic">
                      {winner.quote}
                    </p>
                  </div>
